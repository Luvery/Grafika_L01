...
typedef struct bpatch { // struktura dla wielomiana fragmentu Beziera 3 stopnia
	...
	POINT_3D anchors[4][14]; // siatka 4x14 punktów kontrolnych (anchor)
...
int divs = 14; // number of interpolation (distribution of polygon)
...
GLuint genBezier(BEZIER_PATCH patch, int divs) {
	...
	temp[0] = patch.anchors[0][13];
	temp[1] = patch.anchors[1][13];
	temp[2] = patch.anchors[2][13];
	temp[3] = patch.anchors[3][13];
	temp[4] = patch.anchors[4][13];
	temp[5] = patch.anchors[5][13];
//	temp[6] = patch.anchors[6][13];
...
double x[14] = { 2.0, 1.2, 0.7, 0.6, 0.1, -0.4, -0.45, -0.8, -1.0, -1.5, -1.7, -2.0 ,-2.1 };
double y[4] = { -1.75, -0.50, 0.50, 1.75};
//double z[14] = { -1.8,-1.6, -1.2,-0.3,-0.45,-0.17,0.44,0.9,1.2,2.1,3, 1.7, 1.8, 1.9 };
double z[14] = { 2.2, 1.4, 1.1, 0.6, 0.1, 0.4, 0.45, 0.8, 1.0, 1.5, 1.7, 2.0 , 2.1 };


void initBezier(void) { // 4 po 14
	...
	mybezier.anchors[0][9] = makePoint(x[9], y[0], z[9]);
	mybezier.anchors[0][10] = makePoint(x[10], y[0], z[10]);
	mybezier.anchors[0][11] = makePoint(x[11], y[0], z[11]);
	mybezier.anchors[0][12] = makePoint(x[12], y[0], z[12]);
	mybezier.anchors[0][13] = makePoint(x[13], y[0], z[13]);
	...
	mybezier.anchors[1][9] = makePoint(x[9], y[1], z[9]);
	mybezier.anchors[1][10] = makePoint(x[10], y[1], z[10]);
	mybezier.anchors[1][11] = makePoint(x[11], y[1], z[11]);
	mybezier.anchors[1][12] = makePoint(x[12], y[1], z[12]);
	mybezier.anchors[1][13] = makePoint(x[13], y[1], z[13]);
	...
	mybezier.anchors[2][9] = makePoint(x[9], y[2], z[9]);
	mybezier.anchors[2][10] = makePoint(x[10], y[2], z[10]);
	mybezier.anchors[2][11] = makePoint(x[11], y[2], z[11]);
	mybezier.anchors[2][12] = makePoint(x[11], y[2], z[11]);
	mybezier.anchors[2][13] = makePoint(x[11], y[2], z[11]);
	...
	mybezier.anchors[3][9] = makePoint(x[9], y[3], z[9]);
	mybezier.anchors[3][10] = makePoint(x[10], y[3], z[10]);
	mybezier.anchors[3][11] = makePoint(x[11], y[3], z[11]);
	mybezier.anchors[3][12] = makePoint(x[11], y[3], z[11]);
	mybezier.anchors[3][13] = makePoint(x[11], y[3], z[11]);
...

{
	...
	glClearColor(1.00f, 1.00f, 1.00f, 0.5f); // kolor biały
	glClearDepth(1.0f);
...
void TimerFunction(int value) {
	...
	for (int i = 0; i < 14; i++)
	{
		x[i] = x[i] * sin(14 * 1.e-4*t);
		std::cout << x[i];
	}
	t += 1000;
	for (int i = 0; i < 14; i++)
	{
		z[i] = z[i] * sin(14 * 1.e-4*t);
	}
	t += 1000;
	for (int i = 0; i < 4; i++)
	{
		y[i] = y[i] * sin(14 * 1.e-4*t);
	}
	std::cout << x[0] << "aaaa";
	initBezier();
	InitGL();
	DrawGLScene();
	glutPostRedisplay();
	glutTimerFunc(200000000, TimerFunction, 1);
