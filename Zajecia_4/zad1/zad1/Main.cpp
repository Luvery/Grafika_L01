...
enum
{
	...
	TEXTURE_LENY,                    // tekstura lena
	TEXTURE_LENY_UNC,                // teksura lena nieskompresowana
	TEXTURE_LENY_GRAY,               // tekstura lena_gray
	TEXTURE_LENY_GRAY_UNC,
...
GLuint LENY, LENY_UNC, LENY_GRAY, LENY_GRAY_UNC;
...
void GenerateTextures()
{
	...
	GLboolean error = load_targa("leny.tga", width, height, format, type, pixels);

	// b³¹d odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku leny.tga");
		exit(0);
	}

	// utworzenie identyfikatora tekstury
	glGenTextures(1, &LENY);

	// dowi¹zanie stanu tekstury
	glBindTexture(GL_TEXTURE_2D, LENY);

	// definiowanie tekstury z kompresj¹
	glTexImage2D(GL_TEXTURE_2D, 0, GL_COMPRESSED_RGB, width, height, 0, format, type, pixels);

	// utworzenie identyfikatora tekstury
	glGenTextures(1, &LENY_UNC);

	// dowi¹zanie stanu tekstury
	glBindTexture(GL_TEXTURE_2D, LENY_UNC);
	...
	error = load_targa("leny_gray.tga", width, height, format, type, pixels);

	// b³¹d odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku leny_gray.tga");
		exit(0);
	}

	// utworzenie identyfikatora tekstury
	glGenTextures(1, &LENY_GRAY);

	// dowi¹zanie stanu tekstury
	glBindTexture(GL_TEXTURE_2D, LENY_GRAY);

	// definiowanie tekstury z kompresj¹
	glTexImage2D(GL_TEXTURE_2D, 0, GL_COMPRESSED_LUMINANCE, width, height, 0, format, type, pixels);

	// utworzenie identyfikatora tekstury
	glGenTextures(1, &LENY_GRAY_UNC);

	// dowi¹zanie stanu tekstury
	glBindTexture(GL_TEXTURE_2D, LENY_GRAY_UNC);
...
void Menu(int value)
{
	...
	case TEXTURE_LENY_GRAY_UNC:
	{
		texture = LENY_GRAY_UNC;
		DisplayScene();
	}
	break;
...
int main(int argc, char *argv[])
{
	...
	glutAddMenuEntry("leny.tga (kompresja)", TEXTURE_LENY);
	glutAddMenuEntry("leny.tga (bez kompresji)", TEXTURE_LENY_UNC);
	glutAddMenuEntry("leny_GRAY.tga (kompresja)", TEXTURE_LENY_GRAY);
	glutAddMenuEntry("leny_GRAY.tga (bez kompresji)", TEXTURE_LENY_GRAY_UNC);
